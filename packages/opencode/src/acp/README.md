...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for CodeMAD.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal CodeMAD sessions
...
codemad acp --cwd /path/to/project
```
...
- **Terminal Integration**: Full terminal support via CodeMAD's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | codemad acp
```
...
ACP sessions map cleanly to CodeMAD's internal session model:
